...
def get_prediction():
    ...
    predicted_temp = model.predict(pd.concat([X, pd.DataFrame({'water_temp': water_temp}, index=[0])], axis=1).values)
    return predicted_temp[0], today + datetime.timedelta(days=1)

def get_prediction_for_day(day, prev_temp):
    model = pickle.load(open(base_path + '\\models\\model.pkl', 'rb'))
    X = pd.DataFrame(weather(day), index=[0])
    X = prepare(X, day)
    predicted_temp = model.predict(pd.concat([X, pd.DataFrame({'water_temp': prev_temp}, index=[0])], axis=1).values)
